...
class SecretChat(Object):
    ...
    Args:
        ...
        layer (:obj:`int`):
            Secret chat layer; determines features supported by the chat partner's applicationNested text entities and underline and strikethrough entities are supported if the layer >= 101, files bigger than 2000MB are supported if the layer >= 143
    ...
    def __init__(self, id, user_id, state, is_outbound, key_hash, layer, **kwargs):
    ...
    def read(q: dict, *args) -> "SecretChat":
        ...
        layer = q.get('layer')
        return SecretChat(id, user_id, state, is_outbound, key_hash, layer)
